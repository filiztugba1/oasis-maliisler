...
const StudentHistorySnack: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios.post<HistoryResponse>('http://api-oasis.localhost/maliisler/maliisler/student-history-list',{
          stu_id:localStorage.getItem('search-student-id')
            }).then((res)=>{
                // setLoading(false);
                if(res.status===200)
                {
                  setHistlistx(res.data.data);
                  setFilteredDataHist(res.data.data);
                    setIsApi(false);
                }
              console.log();
            }).catch(err=>{
      ...
      axios.post<ScholarshipHistoryResponse>('http://api-oasis.localhost/maliisler/maliisler/scholarship-history-list',{
        stu_id:localStorage.getItem('search-student-id')
          }).then((res)=>{
              // setLoading(false);
              if(res.status===200)
              {
                setHistlistScolaar(res.data.data);
                setFilteredDataScolaarHist(res.data.data);
                  setIsApi(false);
              }
            console.log();
          }).catch(err=>{
